...
func TestRedir_PreambleAndBidirectionalCopy(t *testing.T) {
	...
	h := hostWithTool(t, rt)
	tok := ""
	for _, e := range h.ServerConfigForSession("conv-9")[0].Stdio.Env {
		if e.Name == "TEST_TOK" {
			tok = e.Value
		}
	}
...
func TestPump_NetPipeNoCloseWrite(t *testing.T) {
	// net.Pipe does not implement CloseWrite; exercise the false branch of
	// halfCloseWrite.
	a, b := net.Pipe()
	go func() {
...
func TestRunRedir_Happy(t *testing.T) {
	h := hostWithTool(t, &recordTool{})
	tok := ""
	for _, e := range h.ServerConfigForSession("c1")[0].Stdio.Env {
		if e.Name == "TEST_TOK" {
			tok = e.Value
		}
	}
