...
func TestTrackerZeroValueIsPending(t *testing.T) {
	// The zero value must report the truth, because a relay reads it
	// before the probe goroutine has been scheduled.
	var tr probe.Tracker
	if got := tr.Status(); got != probe.StatusPending {
		t.Fatalf("zero Tracker status = %v, want pending", got)
	}
}
...
func TestTrackerRecordsSuccess(t *testing.T) {
	...
	if got := tr.Status(); got != probe.StatusProbed {
		t.Fatalf("status = %v, want probed", got)
	}
}

func TestTrackerRecordsBudgetExhaustion(t *testing.T) {
	// Exhausting the budget must NOT leave the tracker pending: a relay
	// would then tell the user "the agent is still starting" forever.
	var tr probe.Tracker
	clk := newClock()

	err := probe.Models(context.Background(), probe.Config{
		Prober:  &flakyProber{failures: 1 << 20},
		Tracker: &tr,
	...
	if err == nil {
		t.Fatal("Models: want error once the budget is exhausted")
	}
	if got := tr.Status(); got != probe.StatusFailed {
		t.Fatalf("status = %v, want failed", got)
	}
}

func TestTrackerRecordsCancellation(t *testing.T) {
	var tr probe.Tracker
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
...
func TestStatusString(t *testing.T) {
	for _, tc := range []struct {
		...
		{probe.StatusPending, "pending"},
		{probe.StatusProbed, "probed"},
		{probe.StatusFailed, "failed"},
		{probe.Status(7), "Status(7)"},
	} {
		if got := tc.s.String(); got != tc.want {
			t.Fatalf("Status(%d).String() = %q, want %q", int32(tc.s), got, tc.want)
		}
	}
}
