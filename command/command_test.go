...
func (c *fakeCtrl) StatusFor(string) SessionStatus      { return c.status }
...
func TestStatus(t *testing.T) {
	...
	b2 := withCtrl(&fakeCtrl{status: SessionStatus{EffectiveModel: "m", ModelsAvailable: 1}})
	g := hb(b2, "!whoami")
...
func TestRelayAlias(t *testing.T) {
	...
	b := withCtrl(&fakeCtrl{
		status: SessionStatus{EffectiveModel: "opus", HasSession: true, ModelsAvailable: 7},
		relayInfo: RelayInfo{
			Version:         "9.9.9",
			Uptime:          "3h2m1s",
	...
	b2 := withCtrl(&fakeCtrl{relayInfo: RelayInfo{ModelsAvailable: 1}})
